...
class similarity_index_of_label_graph_class(object):
    ...
    def __convert_graph_to_node_frequence(self, G, weight = None) -> dict:
        ...
        if not nx_cls_func.is_empty(G):
            for nbunch_of_component in connected_components(nx_cls_func.to_undirected(G)):
                ...
                if number_of_nodes_for_component > 1:
                    component_subgraph_of_G = nx_cls_func.subgraph(G, nbunch_of_component)  #根据子网节点定位一个子网
    ...
    def __call__(self, G1, G2, weight = None) -> float:
        ...
        dict_of_node_frequence_1 = self.__convert_graph_to_node_frequence(G1, weight)
        dict_of_node_frequence_2 = self.__convert_graph_to_node_frequence(G2, weight)
        ...
        for _node in (set(G2.nodes) - set(G1.nodes)):
            dict_of_node_frequence_1.update({_node: 0})
        for _node in (set(G1.nodes) - set(G2.nodes)):
            dict_of_node_frequence_2.update({_node: 0})
        #End of the paragraph
        
        frequency_vector_of_graph_1 = frequency_dict_to_label_order_vector(dict_of_node_frequence_1)
        frequency_vector_of_graph_2 = frequency_dict_to_label_order_vector(dict_of_node_frequence_2)
